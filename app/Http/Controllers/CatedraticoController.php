...
class CatedraticoController extends Controller
{
    ...
    public function notas($curso_id) {
        ...
        '
            SELECT EST.ID ID_ESTUDIANTE, EST.NOMBER ESTUDIANTE FROM ASIGNACION_CURSO AG
            INNER JOIN PAGO PG ON AG.ID = PG.ID
            INNER JOIN ESTUDIANTE EST ON PG.ID_ESTUDIANTE = EST.ID
            INNER JOIN PRE_REQ_CURSO REQ ON AG.ID_PRE_REQ_CURSO = REQ.ID
            INNER JOIN CURSO CS ON REQ.ID_CURSO = CS.ID
            WHERE REQ.ID_CURSO = ?
        '
        , [$curso_id]);
