...
<div class="py-8 px-4 mx-auto max-w-2xl lg:py-20">
    ...
    <form action="{{ route('inscripcion.store') }}" method="POST" class="bg-white rounded-lg shadow-lg p-6">
        @csrf
        <h2 class="mb-4 text-xl font-medium text-slate-700">Inscripción</h2>

        <input type="text" name="id_user" id="id_user" value="{{ $user->id + 1 }}" hidden>

        <div class="mb-6">
            <span class="block text-sm font-medium text-slate-700">Nombre del estudiante</span>
        ...
        <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
            <div class="mb-6">
                ...
                <select name="id_carrera" id="id_carrera" class="mt-1 block w-full px-3 py-2 bg-white border border-slate-300 rounded-md text-sm shadow-sm placeholder-slate-400
                focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500
                disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 disabled:shadow-none
              " required/>
                    @foreach ($carrera as $detail)
                        <option value="{{ $detail->id }}">{{ $detail->carrera }}</option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
            <div class="mb-6">
                ...
                focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500
                disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 disabled:shadow-none
              " value="Umg1234+" readonly required/>
            </div>
        ...
        <a href="{{ url('/inscripcion') }}" class="bg-red-700 text-white py-2 px-4 rounded dark:bg-red-700 dark:hover:bg-red-800">
            Regresar
        </a>
    </form>
</div>
